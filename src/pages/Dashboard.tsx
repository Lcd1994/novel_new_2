...
import { Plus, Clock, Sparkles, FileText, Download, Settings, Trash2 } from 'lucide-react';
import { useProjectStore } from '@/stores/projectStore';

export default function Dashboard() {
  const navigate = useNavigate();
  const { projects, createProject, setCurrentProject, deleteProject, getProjectStats, exportProject } = useProjectStore();
  ...
  const handleExport = (e: React.MouseEvent, projectId: string, title: string, format: 'md' | 'txt') => {
    e.stopPropagation();
  ...
  const handleDelete = (e: React.MouseEvent, projectId: string) => {
    e.stopPropagation();
    if (confirm('确定要删除这个项目吗？所有内容将无法恢复。')) {
      ...
      <header className="border-b border-ink-500/30 bg-ink-700/80 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => navigate('/settings')} className="ink-button text-sm">
              <Settings className="w-4 h-4 mr-1" />
              设置
            </button>
            <button onClick={() => setShowModal(true)} className="ink-button-primary text-sm">
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        ...
        {projects.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-ink-300 mb-6">创建一个新项目，让 AI 为你编织精彩故事</p>
            <button onClick={() => setShowModal(true)} className="ink-button-primary">
              创建第一个小说项目
            </button>
          </div>
        ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-4">
                      <div className="bg-ink-600/30 rounded-lg p-2 text-center">
                        <p className="text-lg font-serif text-amber-gold">{stats.chapterCount}</p>
                        <p className="text-xs text-ink-400">章节</p>
                      </div>
                      <div className="bg-ink-600/30 rounded-lg p-2 text-center">
                        <p className="text-lg font-serif text-amber-gold">{stats.totalWords}</p>
                        <p className="text-xs text-ink-400">字数</p>
                      </div>
                      <div className="bg-ink-600/30 rounded-lg p-2 text-center">
                        <p className="text-lg font-serif text-amber-gold">{stats.charCount}</p>
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={(e) => handleExport(e, project.id, project.title, 'md')}
                      ...
                      <button
                        onClick={(e) => handleExport(e, project.id, project.title, 'txt')}
                        className="ink-button flex-1 text-sm"
                      >
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="ink-card w-full max-w-md p-6 animate-fade-in">
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowModal(false)} className="ink-button flex-1">
                取消
              </button>
              <button onClick={handleCreate} disabled={!newTitle.trim()} className="ink-button-primary flex-1">
                创建
              </button>
            </div>
          </div>
